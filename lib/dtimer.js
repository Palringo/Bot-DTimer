...
const logger = new LoggerFactory().getLogger('DTimer'); // eslint-disable-line no-unused-vars
...
DTimer.prototype._onSubMessage = function(chId, msg) { // eslint-disable-line func-names
    try {
        ...
        logger.error('Malformed message:', msg, '- Error', error);
        this.emit('error', error);
    }
};

DTimer.prototype.join = async function() { // eslint-disable-line func-names
    const self = this;
    ...
    const now = await self._redisTime();

    const replies = await self._pub.multi()
...
DTimer.prototype._redisTime = async function() { // eslint-disable-line func-names
    return this._pub.time()
        .then((result) => result[0] * MILLISECOND + Math.floor(result[1] / MILLISECOND));
};

DTimer.prototype.leave = async function() { // eslint-disable-line func-names
    if (!this._sub) {
    ...
    try {
        ...
        logger.error('Error on leave', error);

    } finally {
        await new Promise((resolve, reject) => {
            self._sub.unsubscribe(self._id, (err) => {
                if (err) { return reject(err); }
                resolve();
            });
        });
...
DTimer.prototype.post = async function(ev, delay) { // eslint-disable-line func-names
    const self = this;
...
DTimer.prototype.peek = async function(evId) { // eslint-disable-line func-names
    const self = this;
    ...
    if (replies[0][1] === null || replies[1][1] === null) {
        return null;
    }

    return {
        remaining: Math.max(parseInt(replies[0][1]) - now, 0),
        timer: JSON.parse(replies[1][1]),
    };
};

DTimer.prototype.cancel = async function(evId) { // eslint-disable-line func-names
    const self = this;
...
DTimer.prototype.confirm = async function(evId) { // eslint-disable-line func-names
    const self = this;
...
DTimer.prototype.changeDelay = async function(evId, delay) { // eslint-disable-line func-names
    const self = this;
...
DTimer.prototype._onTimeout = async function() { // eslint-disable-line func-names
    const self = this;
    this._timer = null;
    ...
    try {
        ...
        try {
            ...
            const replies = await self._pub.update(
                self._keys.gl,
                self._keys.ch,
                self._keys.ei,
                self._keys.ed,
            ...
            if (replies[0].length > 0) {
                replies[0].forEach((sev) => {
                    ...
                    try {
                        ...
                        logger.error(`${self._id}: fail to parse event:`, error);
                        return;
                    }
                    self.emit('event', ev);
                });
            ...
            logger.error(`${self._id}: update failed:`, error);
        ...
        logger.error(`${self._id}: _onTimeout failure:`, err);
        self.emit('error', err);
    }
};

DTimer.prototype.upcoming = async function(option) { // eslint-disable-line func-names
    const self = this;
    ...
    if (typeof option === 'object') {
        _option = _und.defaults(option, defaultOptions);
    } else {
    ...
    return await self._pub.zrangebyscore(args)
        .then((zrangeResults) => {
